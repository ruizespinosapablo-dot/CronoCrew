...
export default function Clock({ emp }) {
  ...
  // Descanso PLANIFICADO del día: el de la citación de ClapCrew si existe, o el
  // de contrato. Se congela en el registro (cited_brk) para que, aunque el
  ...
  const doFich = (type, method) => {
    ...
    if (type === 'in') {
      upsertRec({
        ...
        method: methodLabel, citedIn, citedOut, citedBrk,
      });
    } else {
  ...
  const doLibranza = () => {
    upsertRec({
      ...
      method: 'Libranza', citedIn, citedOut, citedBrk, libranza: true,
    });
  };
  ...
  const doFestivoNoTrabajado = () => {
    upsertRec({
      ...
      method: 'Sistema', citedIn, citedOut, citedBrk, absence: 'festivo',
    });
  };
  ...
  const savePerm = () => {
    ...
    const base = recs.find(r => r.eid === emp.id && r.date === TODAY) || {
      ...
      brk: emp.brk, obs: '', status: 'draft', method: '—', citedIn, citedOut, citedBrk,
    };
  ...
  const saveBrk = () => {
    ...
    const base = recs.find(r => r.eid === emp.id && r.date === TODAY) || {
      id: crypto.randomUUID(), eid: emp.id, date: TODAY, entry: '', exit: '',
      obs: '', status: 'draft', method: '—', citedIn, citedOut, citedBrk,
    };
  ...
  const saveObs = () => {
    ...
    upsertRec({
      ...
      method: '—', citedIn, citedOut, citedBrk,
    });
    setObs('');
